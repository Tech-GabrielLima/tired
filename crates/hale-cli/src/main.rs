...
use hale_compiler::compile;
use hale_runtime::{fetch_json, infer, RecordMode, Runtime, Value};
...
fn usage() {
    eprintln!(
        "hale — a compiled DSL for consuming HTTP APIs\n\n\
         usage:\n\
         \x20 hale run     <file> [--flow NAME [arg ...]] [--show-plan] [--metrics]\n\
         \x20                      [--record <rec.json>] [--replay <rec.json>]\n\
         \x20 hale check   <file>\n\
         \x20 hale fmt     <file> [--write]\n\
         \x20 hale test    <file>\n\
         \x20 hale explain <file>\n\
         \x20 hale inspect <url|file.json> [TypeName]   # infer hale types from JSON\n\
         \x20 hale schema  <file>                       # export types/contracts as JSON Schema\n\
         \x20 hale serve   <file> [Server] [--port N]   # run a `server` block over HTTP\n\
         \x20 hale replay  <rec.json> <file>            # re-run offline from a recording\n\
         \x20 hale lsp                                  # run the language server (stdio)"
    );
}
...
fn report(diags: &hale_syntax::Diagnostics, src: &str, path: &str) -> bool {
    if !diags.is_empty() {
...
async fn cmd_run(args: &[String]) -> ExitCode {
    let Some(path) = args.first() else {
        eprintln!("error: `hale run` needs a file");
        return ExitCode::FAILURE;
    };
...
/// `hale inspect <url|file.json> [TypeName]` — infer hale types from a JSON sample.
async fn cmd_inspect(args: &[String]) -> ExitCode {
    let Some(target) = args.first() else {
        eprintln!("error: `hale inspect` needs a URL or a .json file");
        return ExitCode::FAILURE;
    };
    let name = args
        .get(1)
...
async fn cmd_serve(args: &[String]) -> ExitCode {
    let Some(path) = args.first() else {
        eprintln!("error: `hale serve` needs a file");
        return ExitCode::FAILURE;
    };
    let Some(src) = read(path) else {
        return ExitCode::FAILURE;
...
/// `hale schema <file>` — export the program's types/contracts as JSON Schema.
fn cmd_schema(args: &[String]) -> ExitCode {
    let Some(path) = args.first() else {
        eprintln!("error: `hale schema` needs a file");
        return ExitCode::FAILURE;
    };
    let Some(src) = read(path) else {
        return ExitCode::FAILURE;
    ...
    let title = flag_value(args, "--title").unwrap_or_else(|| "hale types".into());
    match hale_runtime::schema::to_json_schema(&program, &title) {
        Some(s) => {
            println!("{s}");
            ExitCode::SUCCESS
        }
...
/// `hale replay <rec.json> <file>` — run a program offline against a recording.
async fn cmd_replay(args: &[String]) -> ExitCode {
    let (Some(rec), Some(path)) = (args.first(), args.get(1)) else {
        eprintln!("error: usage: hale replay <rec.json> <file.hale>");
        return ExitCode::FAILURE;
    };
    let Some(src) = read(path) else {
        return ExitCode::FAILURE;
...
fn cmd_check(args: &[String]) -> ExitCode {
    let Some(path) = args.first() else {
        eprintln!("error: `hale check` needs a file");
        return ExitCode::FAILURE;
    };
    let Some(src) = read(path) else {
        return ExitCode::FAILURE;
    };
    let diags = hale_compiler::analyze(&src);
...
fn cmd_fmt(args: &[String]) -> ExitCode {
    let Some(path) = args.first() else {
        eprintln!("error: `hale fmt` needs a file");
        return ExitCode::FAILURE;
    };
    let Some(src) = read(path) else {
        return ExitCode::FAILURE;
    ...
    let formatted = hale_syntax::pretty::program(&program);
...
async fn cmd_test(args: &[String]) -> ExitCode {
    let Some(path) = args.first() else {
        eprintln!("error: `hale test` needs a file");
        return ExitCode::FAILURE;
    };
    let Some(src) = read(path) else {
        return ExitCode::FAILURE;
...
fn cmd_explain(args: &[String]) -> ExitCode {
    let Some(path) = args.first() else {
        eprintln!("error: `hale explain` needs a file");
        return ExitCode::FAILURE;
    };
    let Some(src) = read(path) else {
        return ExitCode::FAILURE;
