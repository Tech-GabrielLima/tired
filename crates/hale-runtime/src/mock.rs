...
//! variant (e.g. `NotFound`) produces a typed failure. Mocks make `hale test` run
//! fully offline and deterministic.

use std::collections::HashMap;

use hale_syntax::ast::{Expr, MockDecl, PathSeg};
...
mod tests {
    ...
    fn engine(src: &str) -> MockEngine {
        ...
        for item in &prog.items {
            if let hale_syntax::ast::Item::Mock(m) = item {
