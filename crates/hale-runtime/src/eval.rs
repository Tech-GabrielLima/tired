...
mod tests {
    ...
    fn parse_expr(src: &str) -> Expr {
        ...
        let (prog, d) = hale_syntax::parse(&format!("let __x = {src}"));
        assert!(!d.has_errors(), "{}", d.render(src, "t"));
        match &prog.items[0] {
            hale_syntax::ast::Item::Stmt(Stmt::Let { value, .. }) => value.clone(),
    ...
    fn pattern_match_binds_payload() {
        ...
        let (prog, _) =
            hale_syntax::parse("flow F() { match e { Err(RateLimit(ms)) => ms _ => other() } }");
        // Reach into the parsed match for its first arm pattern.
        if let hale_syntax::ast::Item::Flow(f) = &prog.items[0] {
            if let Stmt::Expr {
                expr: Expr::Match(m),
                ..
            } = &f.body.stmts[0]
