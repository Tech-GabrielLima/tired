//! `hale schema` — export the `type`/`contract` declarations of a program as a
...
//! constraints become `minimum`/`maximum`/`minLength`/… keywords. This makes a hale
...
mod tests {
    ...
    fn emits_schema_with_constraints_and_refs() {
        ...
        "#;
        let (prog, _) = hale_syntax::parse(src);
