...
use hale_runtime::{Runtime, Value};
...
/// Minimal blocking HTTP/1.1 GET for driving the hale server under test.
...
fn build(src: &str) -> Runtime {
    let (compiled, diags) = compile(src, "test.hale");
    assert!(
        !diags.has_errors(),
        "compile errors: {}",
        diags.render(src, "test.hale")
    );
...
fn benchmark_parallel_vs_serial() {
    ...
    println!("\n=== hale parallel-inference benchmark ({n} fetches @ {hop:?}/hop) ===");
