//! `hale-lsp` — a Language Server for hale over stdio (LSP / JSON-RPC).
//!
//! It reuses the real compiler: on every edit it runs `hale_compiler::analyze` and
...
use hale_syntax::ast::{Item, StrPart};
...
impl Server {
        ...
        match method {
            "initialize" => vec![response(
                ...
                json!({
                    ...
                    "serverInfo": { "name": "hale-lsp", "version": env!("CARGO_PKG_VERSION") }
                }),
            )],
            "initialized" => vec![],
    ...
    fn diagnostics(&self, uri: &str) -> Value {
        ...
        let diags = hale_compiler::analyze(&text);
        let items: Vec<Value> = diags
            ...
            .map(|d| {
                ...
                json!({
                    ...
                    "severity": match d.severity { hale_syntax::Severity::Error => 1, _ => 2 },
                    "source": "hale",
                    "message": message
                })
            })
    ...
    fn completions(&self, uri: &str) -> Value {
        let text = self.docs.get(uri).cloned().unwrap_or_default();
        let (program, _) = hale_syntax::parse(&text);
...
fn string_lit(e: &hale_syntax::ast::Expr) -> String {
    if let hale_syntax::ast::Expr::Str { parts, .. } = e {
...
mod tests {
    ...
    fn publishes_diagnostics_on_open() {
        ...
        let open = json!({
            ...
            "params": { "textDocument": { "uri": "file:///x.hale", "text": "fetch GitGub /u -> x\nlog \"hi\"" } }
        });
