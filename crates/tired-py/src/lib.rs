//! Python bindings for TIRED (PyO3, abi3). The Rust compiler + runtime, callable from
//! Python:
//!
//! ```python
//! import tired
//! tired.check('fetch GitGub /u -> x')         # -> ["unknown endpoint `GitGub`", ...]
//! tired.inspect('{"id": 1, "email": "a@b.c"}', "User")   # -> "type User { ... }"
//! tired.run(open("script.tired").read())       # executes the script
//! ```
...
fn is_valid(src: &str) -> bool {
    !tired_compiler::analyze(src).has_errors()
}

/// The inferred parallel-execution plan + request cost (`tired explain`).
#[pyfunction]
fn explain(src: &str) -> PyResult<String> {
    let (compiled, diags) = tired_compiler::compile(src, "<python>");
    match compiled {
        Some(c) => Ok(c.plan()),
...
fn run(src: &str) -> PyResult<Option<String>> {
    ...
    let rt = tired_runtime::Runtime::new(compiled);
...
/// Infer TIRED `type`/`contract` declarations from a JSON sample.
#[pyfunction]
...
fn inspect(json_text: &str, name: &str) -> PyResult<String> {
    ...
    Ok(tired_runtime::infer::infer_types(&j, name))
}

/// Export a program's `type`/`contract` declarations as a JSON Schema string.
#[pyfunction]
#[pyo3(signature = (src, title = "TIRED types"))]
fn json_schema(src: &str, title: &str) -> PyResult<Option<String>> {
    let (program, diags) = tired_syntax::parse(src);
    if diags.has_errors() {
        return Err(PyRuntimeError::new_err(diags.render(src, "<python>")));
    }
    Ok(tired_runtime::schema::to_json_schema(&program, title))
}

#[pymodule]
fn tired(m: &Bound<'_, PyModule>) -> PyResult<()> {
